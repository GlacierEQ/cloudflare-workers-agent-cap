...
export function mint(secret, requestId, capabilities, notAfter, options = {}) {
  return build(secret, requestId, capabilities, notAfter, options);
}

export function fingerprint(tokenValue) {
  return digest({ ...unsignedBody(tokenValue), mac: tokenValue.mac });
}
...
export function verifyDelegation(secret, parent, child) {
  ...
  if (child.parentFingerprint !== fingerprint(parent)) return false;
  if (child.issuerId !== parent.issuerId) return false;
  if (child.delegationDepth !== parent.delegationDepth + 1) return false;
  if (!child.capabilities.every(capability => parent.capabilities.includes(capability))) return false;
  if (child.notBefore < parent.notBefore || child.notAfter > parent.notAfter) return false;
  return true;
}
...
export function delegationReceipt(secret, parent, child) {
  ...
  const body = {
    ...
    issuer_id: child.issuerId,
    child_request_id: child.requestId,
    removed_capabilities: removedCapabilities,
    parent_not_before: parent.notBefore,
    child_not_before: child.notBefore,
    parent_not_after: parent.notAfter,
    child_not_after: child.notAfter,
    parent_depth: parent.delegationDepth,
    child_depth: child.delegationDepth,
  };
  return Object.freeze({
    parentFingerprint: body.parent_fingerprint,
    childFingerprint: body.child_fingerprint,
    issuerId: child.issuerId,
    childRequestId: child.requestId,
    removedCapabilities: Object.freeze(removedCapabilities),
    parentNotBefore: parent.notBefore,
    childNotBefore: child.notBefore,
    parentNotAfter: parent.notAfter,
    childNotAfter: child.notAfter,
    parentDepth: parent.delegationDepth,
    childDepth: child.delegationDepth,
...
export function invoke(secret, tokenValue, capability, now) {
  ...
  if (typeof capability !== "string" || capability.trim() === "" || typeof now !== "number" || !Number.isFinite(now)) {
    return { status: "REFUSE", reason: "INVALID_INVOCATION" };
  }
...
export function escalate() {
  throw new Error("ESCALATION_FORBIDDEN: use issueSubcap with a verified parent");
}
